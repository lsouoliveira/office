...
class Playground extends Scene {
  ...
  async onStart() {
    ...
    this.camera.scale = Math.min(window.innerWidth / (TILE_SIZE * 32), window.innerHeight / (TILE_SIZE * 18))
  ...
  update() {
    if (this.player) {
        this.camera.centerAt(this.player.position.x + this.player.width / 2, this.player.position.y + this.player.height / 2)
    }
  ...
  private onClick(e) {
    ...
    if (this.playgroundMap.contains(tileX, tileY) && !this.playgroundMap.checkCollision(tileX, tileY)) {
