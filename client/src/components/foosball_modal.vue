...
import { FoosballClient } from '../foosball/client'
import { FoosballGame, StageType, WIDTH, HEIGHT, PAD_WIDTH } from '../foosball/game'
...
onMounted(() => {
  ...
  game.value = new FoosballGame(gameRoot.value, handlePadMove, handleUpdate)

  game.value.init()
  game.value.start()

  foosballClient.value = new FoosballClient('ws://localhost:3002', {
    userId: userId.value,
    username: username.value
  })
  // foosballClient.value.connect()

  foosballClient.value.on('session', (data: any) => {
    localStorage.setItem('userId', data.userId)

    userId.value = data.userId
  })

  foosballClient.value.on('room', (roomState: any) => {
  ...
  foosballClient.value.on('game:state', (state: any) => {
    const playerPad = getPlayerPad()

    score.value = state.score
  })

  foosballClient.value.on('game:nextTurn', () => {
    game.value?.reset()
  })

  foosballClient.value.on('game:hit', () => {
  ...
  foosballClient.value.on('user:message', ({ id, username, message, created_at }) => {
  ...
  foosballClient.value.on('user:connected', (user: any) => {
    users.value.push(user)
  })

  foosballClient.value.on('user:disconnected', (user: any) => {
...
const handleUpdate = () => {
  ...
  if (playerPad.userId === pad1.value.userId) {
    foosballClient.value.movePad(playerPad.userId, WIDTH - lastPlayerMove.value - PAD_WIDTH)
  } else {
    foosballClient.value.movePad(playerPad.userId, lastPlayerMove.value)
  }
...
const handleClose = () => {
  foosballClient.value?.disconnect()
  game.value?.destroy()
}
...
const sendMessage = () => {
  ...
  foosballClient.value.sendMessage(message.value)
  message.value = ''
}
</script>
  ...
  <b-modal v-model="model" width="" @close="handleClose" can-cancel="['x', 'escape']">
    <div>
      <div class="flex">
        ...
        <div
          style="width: 896px; height: 480px"
